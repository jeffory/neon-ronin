...
func _initialize() -> void:
	...
	# Platform
	var platform_a := CSGBox3D.new()
	platform_a.name = "PlatformA"
	platform_a.size = Vector3(30, 1, 30)
	platform_a.position = Vector3(-20, 39.5, 0)
	platform_a.use_collision = true
	platform_a.collision_layer = 4  # environment (layer 3)
	platform_a.collision_mask = 0
	platform_a.material = concrete_mat
	rooftop_a.add_child(platform_a)
	...
	# Back wall (north)
	var ds_a_back := CSGBox3D.new()
	ds_a_back.name = "DoorStructA_Back"
	ds_a_back.size = Vector3(3, 3, 0.2)
	ds_a_back.position = Vector3(0, 1.5, -1.4)
	ds_a_back.use_collision = true
	ds_a_back.collision_layer = 4
	ds_a_back.collision_mask = 0
	ds_a_back.material = facade_mat
	door_struct_a.add_child(ds_a_back)
	...
	# Stairwell A — ramp descending from Y=40 to Y=36
	var ramp_a := CSGBox3D.new()
	ramp_a.name = "RampA"
	ramp_a.size = Vector3(2, 0.2, 5)
	ramp_a.position = Vector3(-12, 38, 10.5)
	ramp_a.rotation_degrees = Vector3(25, 0, 0)
	ramp_a.use_collision = true
	ramp_a.collision_layer = 4
	ramp_a.collision_mask = 0
	ramp_a.material = concrete_mat
	nav_region.add_child(ramp_a)
	...
	# Ceiling
	var ceiling_a := CSGBox3D.new()
	ceiling_a.name = "CeilingA"
	ceiling_a.size = Vector3(20, 0.3, 15)
	ceiling_a.position = Vector3(-20, 39.7, 0)
	ceiling_a.use_collision = true
	ceiling_a.collision_layer = 4
	ceiling_a.collision_mask = 0
	ceiling_a.material = office_wall_mat
	office_a.add_child(ceiling_a)
	...
	# Platform
	var platform_b := CSGBox3D.new()
	platform_b.name = "PlatformB"
	platform_b.size = Vector3(25, 1, 25)
	platform_b.position = Vector3(25, 39.5, 0)
	platform_b.use_collision = true
	platform_b.collision_layer = 4
	platform_b.collision_mask = 0
	platform_b.material = concrete_mat
	rooftop_b.add_child(platform_b)
	...
	var ds_b_back := CSGBox3D.new()
	ds_b_back.name = "DoorStructB_Back"
	ds_b_back.size = Vector3(3, 3, 0.2)
	ds_b_back.position = Vector3(0, 1.5, -1.4)
	ds_b_back.use_collision = true
	ds_b_back.collision_layer = 4
	ds_b_back.collision_mask = 0
	ds_b_back.material = facade_mat
	door_struct_b.add_child(ds_b_back)
	...
	# Stairwell B — ramp descending from Y=40 to Y=36
	var ramp_b := CSGBox3D.new()
	ramp_b.name = "RampB"
	ramp_b.size = Vector3(2, 0.2, 5)
	ramp_b.position = Vector3(20, 38, 7.5)
	ramp_b.rotation_degrees = Vector3(25, 0, 0)
	ramp_b.use_collision = true
	ramp_b.collision_layer = 4
	ramp_b.collision_mask = 0
	ramp_b.material = concrete_mat
	nav_region.add_child(ramp_b)
	...
	# Ceiling B
	var ceiling_b := CSGBox3D.new()
	ceiling_b.name = "CeilingB"
	ceiling_b.size = Vector3(15, 0.3, 12)
	ceiling_b.position = Vector3(25, 39.7, 0)
	ceiling_b.use_collision = true
	ceiling_b.collision_layer = 4
	ceiling_b.collision_mask = 0
	ceiling_b.material = office_wall_mat
	office_b.add_child(ceiling_b)
	...
	facade_a.size = Vector3(30, 40, 30)
	facade_a.position = Vector3(-20, 19.5, 0)
	...
	facade_b.size = Vector3(25, 40, 25)
	facade_b.position = Vector3(25, 19.5, 0)
	...
	kz_shape.size = Vector3(200, 1, 200)
	...
	var pickup_positions: Array = [
		...
		Vector3(3.75, 40.5, 0),  # Sky bridge
